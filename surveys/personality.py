...
def read_surveys(data_folder = "data"):
    qs = [1,3,4]
    groups = ['ij', 'c']
    return pd.concat([reduce(lambda a,b: pd.merge(a,b,on='user_id'),
                     [read_df(g, q, data_folder) for q in qs]).assign(group = g)
...
def get_questions(comps, data_folder, thresh = 0.01):
    comps = comps.T.tolist()
    a = pd.read_csv(path.join(data_folder, "educatalyst/Auxil/q3_key_teique.csv"))
    b = pd.read_csv(path.join(data_folder, "educatalyst/Auxil/q4_key_grit.csv"))
